...
/** Intro preview — the three USDC prizes, then flip to ?. */
const INTRO_USDC_PREVIEW = [
  { tone: "lavender" as const, label: "1 USDC" },
  { tone: "mint" as const, label: "0.05 USDC" },
  { tone: "blush" as const, label: "0.001 USDC" },
];
...
export default function DailyShuffleModal({
  ...
  /** Intro: show USDC amounts, then flip to ?. */
  ...
  function revealLabel(card: ShuffleTheaterCard | null): string {
    if (!card) return "";
    if (card.type === "usdc" && card.amount != null) return `${card.amount} USDC`;
    return card.label;
  }
      ...
      >
                    ...
                    >
                      <TicketShell tone={tone} faceDown={!showFront}>
                        {showFront && display ? (
                          <CardFaceContent
                            glyph={
                              display.type === "usdc" ? null : display.glyph
                            }
        ...
        {(phase === "reveal" || phase === "claiming") && winnerCard ? (
          <div className="daily-shuffle-result">
            ...
            <p className="daily-shuffle-result-amt">
              {outcome?.type === "usdc" && outcome.amount != null
                ? `${outcome.amount} USDC`
                : outcome?.type === "spark"
                  ? "Unlimited plays · 24h"
        ...
        <div className="daily-shuffle-actions">
          {phase === "intro" || phase === "busy" ? (
            ...
            >
              {phase === "busy" ? "Confirm in wallet…" : "Shuffle now · Free"}
            </button>
          ) : null}
