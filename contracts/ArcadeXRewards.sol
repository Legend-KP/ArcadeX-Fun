 ...
 * @notice Base hub: STREAK daily check-in + SHUFFLE signed spins + shared claim path.
 *
 ...
 *      - Treasury reservation for USDC (fixed streak or per-spin won amount)
...
contract ArcadeXRewards is ReentrancyGuard, EIP712, IERC721Receiver {
    ...
    struct Campaign {
        ...
        /// @notice On-chain ceiling for a single shuffle payout (USDC). Ignored for STREAK.
    ...
    function spin(
        ...
        // Resolve targets for storage (USDC pinned; ERC721 uses signed target).
