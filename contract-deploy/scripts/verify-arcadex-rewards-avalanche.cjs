...
const DEPLOYMENT_FILE = path.resolve(
  __dirname,
  "../../deployments/arcadex-rewards-avalanche-mainnet.json"
);
...
require("dotenv").config({
  path: path.resolve(__dirname, "../../../arcadex-celo/.env"),
});
...
async function verifyOnSnowtrace(address, deployment) {
  if (!apiKey) {
    return false;
  }
  ...
  if (submit.status !== "1") {
    throw new Error(
      `Snowtrace submit failed: ${submit.message || JSON.stringify(submit)}`
    );
  }

  const guid = submit.result;
  console.log("Snowtrace GUID:", guid);
  ...
  for (let i = 0; i < 20; i++) {
    ...
    console.log("Snowtrace status:", status.result || status.message);

    if (status.status === "1") {
      console.log(
        "Verified on Snowtrace:",
        `https://snowtrace.io/address/${address}#code`
      );
      return true;
    }
  ...
  throw new Error("Snowtrace verification timed out");
}

async function checkSnowtraceVerified(address) {
...
function saveVerification(deployment, snowtraceUrl) {
  const updated = {
    ...deployment,
    verified: true,
    verifiedAt: new Date().toISOString(),
    verification: {
      snowtrace: snowtraceUrl,
...
async function main() {
  ...
  if (await checkSnowtraceVerified(address)) {
    const snowtraceUrl = `https://snowtrace.io/address/${address}#code`;
    console.log("Already verified on Snowtrace:", snowtraceUrl);
    saveVerification(deployment, snowtraceUrl);
    return;
  }

  let snowtraceOk = false;
  if (apiKey) {
    try {
      snowtraceOk = await verifyOnSnowtrace(address, deployment);
    } catch (err) {
      console.warn(
        "Snowtrace verify failed:",
        err instanceof Error ? err.message : err
      );
    }
  } else {
    console.log("No explorer API key — trying Sourcify.");
  }
  ...
  console.log("Verification recorded:", snowtraceUrl);
}
